...
# --- GDINO point-wise early fusion (SegDINO3D-style) ---
gdino_target_size = (420, 560)  # (H,W), keep ratio with ScanNet 480x640
gdino_in_dim = 256
gdino_out_dim = 32

model = dict(
    # Route-A: reuse Online model implementation (already has GDINO hooks),
    # but run on single-frame SV (T=1) with map_to_rec_pcd disabled.
    ...
    # Disable online tracking / temporal losses for SV training.
    ...
    backbone=dict(
        type='Res16UNet34C',
        # RGB(3) + GDINO point feat (gdino_out_dim)
        in_channels=3 + gdino_out_dim,
        out_channels=96,
        config=dict(dilations=[1, 1, 1, 1], conv1_kernel_size=5, bn_momentum=0.02)),
    # Keep memory module for compatibility (not used when use_mot=False).
    ...
    # GroundingDINO backbone config (loaded once, frozen).
    gdino_backbone=dict(
    ...
    # Point-wise feature fusion: project 3D points to current frame and sample GDINO srcs.
    ...
    train_cfg=dict(log_missing_bboxes=False),
    test_cfg=dict(
        # SV: no online merge / rec mapping; use concat to output current-frame instances.
        merge_type='concat',
        topk_insts=100,
...
train_pipeline = [
    ...
    # Adapt SV sample to Online input format (T=1) and build cam_info/img_size for GDINO.
    ...
    # Cache raw points for 2D-3D projection before any 3D augmentation.
...
# IMPORTANT: input channels changed due to GDINO fusion; do not load old 3-channel checkpoints by default.
load_from = None
